; =============================================================================
; zcode-assistant 自定义 NSIS 安装器模板
; -----------------------------------------------------------------------------
...
;
;   2. 卸载 Delete 主程序加 /REBOOTOK：进程占用无法立即删除时登记为重启后
...
Section Install
  ...
  ; Copy external binaries (Tauri 按 workspace [[bin]] 目标自动注入 binaries 列表，
  ; 展开为绝对路径 File 指令；zcode-assistant 目前单 binary，此循环为空)
  {{#each binaries}}
...
Section Uninstall
  ...
  ; zcode 改动: 主程序加 /REBOOTOK，进程占用无法立即删除时登记重启后删除
  ...
  ; Delete external binaries (zcode-assistant 单 binary，此循环为空；保留 /REBOOTOK 兜底)
  {{#each binaries}}
